<template>
  <PageContainer title="VIP订单列表" :subtitle="'共 ' + total + ' 条记录'">
    <DataTableSection>
      <template #search>
...
const columns: DataTableColumns = [
  { title: '订单号', key: 'orderNo', width: 180, render: (row: any) => h('span', { style: 'font-family:monospace;font-size:12px' }, row.orderNo) },
  {
    ...
    render(row: any) {
      return h('div', null, [
        ...
        h('div', { style: 'font-size:12px;color:var(--color-text-muted)' }, row.userPhone),
      ])
    }
  },
  { title: '套餐', key: 'planName', width: 100, render: (row: any) => h(NTag, { size: 'small', bordered: false, type: 'warning' }, () => row.planName) },
  {
    ...
    key: 'amount',
    width: 120,
    render: (row: any) => h('span', { style: 'font-size:16px;font-weight:600;color:var(--color-primary)' }, `¥${row.amount}`)
  },
